    ...
    if associaton_type == 'company-company':
        ...
        for line in associations:
            ...
            try:
                ...
                elif contact_list[contact] == None: # if conatct is not in dictionary then ...
...
def data_to_dict(type, directory): #takes data from xlsx file and returns it in a dictionary
    ...
    try:
        if type == "associations":    
            ...
            id_column = ws['A'] # ids
            name_column = ws['B'] # names
            state_column = ws['C'] # states

            # iterates over 3 lists and executes 
            # 2 times as len(value)= 2 which is the
            # minimum among all the three 
            for (id, name, state) in zip(id_column, name_column, state_column):
                try:
                    company_name = name.value
                    # + "*"+ state.value
                    if name.value in data:
                        if isinstance(data[company_name], list):
                            arr = data[name.value]
                            arr.append(id.value)
                            data.update( {company_name : arr} )
                        else:
                            arr = [data[company_name]]
                            arr.append(id.value)
                            data.update( {company_name : arr} )
                    else:
                        data.update( {company_name : id.value} )
                except:
                        data.update( {company_name : id.value} ) 
